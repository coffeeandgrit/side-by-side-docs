...
module.exports = exports = function(source, destination) {
    ...
    return sections(source)
        .then(function(sections) {
            ...
            context.navigationLinks = _.map(context.sections, function(section) {
                ...
                if (!!section.topics && section.topics.length > 1) {
                    logger.silly('section %s has %s topics', section.name, section.topics.length);
                    topics = _.map(section.topics, function(topic) {
                        logger.silly('\t\tsetting up anchor:  %s -> %s', topic.basename, topic.basename.replace(/[^a-z0-9\-\_]/gi, '_'));
                        return {
                            name: topic.name,
                            anchor: topic.anchor
                        }
        ...
        .then(function() {
            ...
            return fs.writeFileAsync(path.resolve(destination, 'index.html'), context.output);
        })
        .then(function() {
            return context;
        });
